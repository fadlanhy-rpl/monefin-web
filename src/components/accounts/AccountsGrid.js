        ...
        {accounts.map((acc, index) => {
          ...
          if (acc.type === "bank-primary") {
            return (
              <div 
                key={acc.id} 
                className="bg-gradient-to-br from-[#00685F] via-[#008A7E] to-[#004D46] p-4 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] text-white flex flex-col justify-between h-56 sm:h-72 shadow-2xl shadow-[#00685F]/35 relative overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-3xl group"
              ...
              >
                ...
                <div className="relative z-10 mt-1 sm:mt-2">
                  ...
                  <h3 className="text-xl sm:text-4xl font-extrabold mt-1 sm:mt-1.5 tracking-tight">Rp {acc.balance.toLocaleString("id-ID")}</h3>
                </div>
                ...
                <div className="relative z-10 flex justify-between items-end border-t border-white/10 pt-2.5 sm:pt-4 mt-1 sm:mt-2">
                  <div className="min-w-0">
                    ...
                    <p className="font-extrabold text-[10px] sm:text-sm tracking-wide mt-0.5 truncate">{acc.holder || "AKHMAD MAARIZ"}</p>
                  </div>
                  {/* Card Brand */}
          ...
          if (acc.type === "bank-dark") {
            return (
              <div 
                key={acc.id} 
                className="bg-gradient-to-br from-[#1E1E1E] via-[#2F2F2F] to-[#121212] p-4 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] text-white flex flex-col justify-between h-56 sm:h-72 shadow-xl relative overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl group border border-white/5"
                style={{ animationDelay: `${(index + 1) * 80}ms` }}
              >
                <div className="relative z-10 flex justify-between items-start">
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          ...
          if (acc.type === "wallet") {
            return (
              <div 
                key={acc.id} 
                className="bg-white p-4 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between h-48 sm:h-64 hover:shadow-lg transition-all duration-300 group relative overflow-hidden"
                style={{ animationDelay: `${(index + 1) * 80}ms` }}
              >
                ...
                <div className="flex justify-between items-end relative z-10 mt-1">
                  <div className="min-w-0">
                    ...
                    <h3 className="text-xl sm:text-3xl font-black text-slate-900 mt-1 truncate">Rp {acc.balance.toLocaleString("id-ID")}</h3>
                  </div>
