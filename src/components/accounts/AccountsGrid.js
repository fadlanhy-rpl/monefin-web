        ...
        {accounts.map((acc, index) => {
          ...
          if (acc.type === "bank-primary") {
            return (
              <div 
                key={acc.id} 
                className="bg-gradient-to-br from-[#00685F] via-[#008A7E] to-[#004D46] p-8 rounded-[2.5rem] text-white flex flex-col justify-between h-72 shadow-2xl shadow-[#00685F]/35 relative overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-3xl group"
                style={{ animationDelay: `${(index + 1) * 80}ms` }}
              >
                <div className="relative z-10 flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/10 shrink-0">
                      <Landmark className="w-5.5 h-5.5" />
                    </div>
                    <div>
                      <h4 className="font-extrabold text-lg tracking-wide uppercase leading-none">{acc.name}</h4>
                      <div className="flex items-center gap-1.5 mt-1 select-none">
                        <span className="text-xs text-white/70 font-mono tracking-wider">{acc.number}</span>
                        <button 
                ...
                <div className="relative z-10 w-9 h-7 bg-gradient-to-br from-amber-300 via-yellow-400 to-amber-600 rounded-md border border-yellow-500/20 shadow-sm shrink-0 self-start select-none">
                ...
                <div className="relative z-10 mt-2">
                  <p className="text-[10px] font-bold text-white/50 uppercase tracking-widest leading-none">Available Balance</p>
                  <h3 className="text-4xl font-extrabold mt-1.5 tracking-tight">Rp {acc.balance.toLocaleString("id-ID")}</h3>
                </div>

                <div className="relative z-10 flex justify-between items-end border-t border-white/10 pt-4 mt-2">
                  <div>
                    <p className="text-[9px] font-bold text-white/40 uppercase tracking-wide">Account Holder</p>
                    <p className="font-extrabold text-sm tracking-wide mt-0.5">{acc.holder || "AKHMAD MAARIZ"}</p>
                  </div>
                  {/* Card Brand */}
                  <span className="text-[10px] font-black tracking-widest text-white/30 uppercase italic">GPN</span>
                </div>
          ...
          if (acc.type === "bank-dark") {
            return (
              <div 
                key={acc.id} 
                className="bg-gradient-to-br from-[#1E1E1E] via-[#2F2F2F] to-[#121212] p-8 rounded-[2.5rem] text-white flex flex-col justify-between h-72 shadow-xl relative overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl group border border-white/5"
                style={{ animationDelay: `${(index + 1) * 80}ms` }}
              >
                <div className="relative z-10 flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white/80 border border-white/5 shrink-0">
                      <CreditCard className="w-5.5 h-5.5" />
                    </div>
                    <div>
                      <h4 className="font-extrabold text-lg tracking-wide uppercase leading-none">{acc.name}</h4>
                      <div className="flex items-center gap-1.5 mt-1 select-none">
                        <span className="text-xs text-white/40 font-mono tracking-wider">{acc.number}</span>
                        <button 
                          onClick={() => handleCopy(acc.id, acc.number)}
                ...
                <div className="mt-2">
                  <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest leading-none">Total Savings</p>
                  <h3 className="text-4xl font-extrabold mt-1.5 tracking-tight">Rp {acc.balance.toLocaleString("id-ID")}</h3>
                </div>

                <div className="flex justify-between items-center pt-2 mt-2">
                  <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-1.5 rounded-xl select-none">
                    <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                    <span className="text-[9px] font-bold text-white/60 uppercase">Status: {acc.status || "Active"}</span>
                  </div>
                  {/* MasterCard Card logo */}
                  <div className="flex gap-0.5 select-none opacity-40 group-hover:opacity-75 transition-opacity">
                    <div className="w-5.5 h-5.5 bg-red-500 rounded-full"></div>
                    <div className="w-5.5 h-5.5 bg-amber-500 rounded-full -ml-3"></div>
                  </div>
                </div>
          ...
          if (acc.type === "wallet") {
            return (
              <div 
                key={acc.id} 
                className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between h-64 hover:shadow-lg transition-all duration-300 group relative overflow-hidden"
                style={{ animationDelay: `${(index + 1) * 80}ms` }}
              >
                <div className="flex justify-between items-start relative z-10">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-brand-50 rounded-2xl flex items-center justify-center text-[#00685F] shadow-inner shrink-0 transition-transform duration-300 group-hover:scale-105 group-hover:rotate-1">
                      <Smartphone className="w-7 h-7" />
                    </div>
                    <div>
                      <h4 className="font-extrabold text-xl text-slate-900 tracking-tight leading-tight">{acc.name}</h4>
                ...
                <div className="flex justify-between items-end relative z-10">
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Balance</p>
                  ...
                  <div className="flex -space-x-2.5 select-none">
                    {(acc.wallets || ["GP", "OV"]).map((w, wIdx) => (
                      <div 
                        key={wIdx} 
                        className={`w-8 h-8 border-2 border-white rounded-full flex items-center justify-center text-[8px] font-black uppercase tracking-tighter shadow-sm transition-transform duration-300 group-hover:translate-x-0.5 ${
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start relative z-10">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-300 shadow-inner shrink-0 transition-transform duration-300 group-hover:scale-105 group-hover:-rotate-1">
              ...
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">In Hand</p>
                <h3 className="text-3xl font-black text-slate-900 mt-2">Rp {acc.balance.toLocaleString("id-ID")}</h3>
              </div>
              
              <div className="flex justify-between items-center text-[10px] font-bold text-gray-300 border-t border-slate-50 pt-4 select-none relative z-10">
