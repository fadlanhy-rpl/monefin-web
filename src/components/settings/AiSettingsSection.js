...
import {
  ...
  AlertTriangle, ShieldCheck, Loader2, X,
  ...
  return (
    <>
      <div className="bg-white p-5 sm:p-7 md:p-9 lg:p-10 rounded-[2rem] sm:rounded-[2.5rem] border border-slate-200/80 shadow-xs space-y-8 sm:space-y-10 transition-all duration-300">
        ...
        {/* Master Toggle */}
        <div className="flex items-center justify-between gap-4 p-4 sm:p-5 bg-slate-50/80 rounded-2xl border border-slate-200/70 shadow-2xs">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${aiEnabled ? "bg-[#00685F] text-white shadow-md shadow-[#00685F]/20" : "bg-slate-200 text-slate-500"}`}>
          ...
          <button
            ...
            className={`relative w-12 h-6 rounded-full transition-colors duration-200 shrink-0 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00685F] ${aiEnabled ? "bg-[#00685F]" : "bg-slate-300"}`}
            role="switch"
            aria-checked={aiEnabled}
            aria-label={aiEnabled ? "Disable AI Chatbot" : "Enable AI Chatbot"}
          >
            <span
        ...
        <div className="flex items-start gap-3 p-4 sm:p-4.5 bg-teal-50/60 border border-teal-200/70 rounded-2xl text-xs text-teal-900 shadow-2xs">
          <Info className="w-4 h-4 shrink-0 mt-0.5 text-[#00685F]" />
          <span className="leading-relaxed font-medium">
            {language === "id"
              ? "Saat AI aktif, fitur Smart Insight di setiap halaman (Dashboard, Kategori, Budget, Akun, Goals) akan menggunakan AI Anda untuk insight yang lebih personal. Saat tidak aktif, MoneFin Engine memberikan insight berbasis logika analisis data internal."
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div ref={providerRef} className="relative space-y-1.5">
              ...
              >
                <span className={provider ? "text-slate-900 font-extrabold" : "text-slate-400 font-medium"}>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-xs sm:text-sm font-black uppercase tracking-wider text-slate-700">API Key</h4>
            {!maskedKey && (
              <button
                type="button"
                onClick={async () => {
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <div className="relative flex-1 min-w-0">
              <input
                ...
                className={`w-full min-h-[48px] border border-slate-200/80 rounded-2xl px-4 py-3 sm:px-4.5 sm:py-3.5 text-xs sm:text-sm pr-11 focus:outline-none focus:ring-4 focus:ring-[#00685F]/10 focus:border-[#00685F] transition font-mono ${!showApiKey && maskedKey && apiKey === "" ? "text-slate-400 bg-slate-50/80 cursor-pointer" : "bg-white text-slate-800"}`}
              />
              ...
              <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                {apiKey === "" && !showApiKey && (
                  <button
                    ...
                    className="min-h-[46px] flex items-center gap-1.5 px-4 py-2.5 text-xs font-black text-amber-700 bg-amber-50 border border-amber-200/80 rounded-2xl hover:bg-amber-100 transition cursor-pointer shadow-2xs"
                ...
                {apiKey === "" && (
                  <button
                    ...
                    className="min-h-[46px] flex items-center gap-1 px-4 py-2.5 text-xs font-black text-slate-700 bg-slate-100 border border-slate-200/80 rounded-2xl hover:bg-slate-200 transition cursor-pointer"
                  >
                ...
                {apiKey !== "" && (
                  <button
                    ...
                    className="min-h-[46px] flex items-center gap-1 px-4 py-2.5 text-xs font-black text-slate-500 hover:text-slate-800 bg-slate-100 rounded-2xl transition cursor-pointer"
                    title={language === "id" ? "Batal ganti key" : "Cancel key change"}
                  >
                    <X className="w-4 h-4" />
                    <span>{language === "id" ? "Batal" : "Cancel"}</span>
                  </button>
                )}
              </div>
            )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <button
            ...
            className="min-h-[44px] flex items-center justify-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-extrabold rounded-2xl shadow-xs transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer active:scale-[0.98]"
          >
            {testStatus === "loading"
          ...
          {testStatus === "error" && (
            <div className="flex items-center gap-2 text-xs font-bold text-rose-700 bg-rose-50 border border-rose-200 px-3.5 py-2 rounded-xl max-w-sm">
        ...
        <div className="pt-4 border-t border-slate-100 flex justify-end">
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full sm:w-auto min-h-[44px] flex items-center justify-center gap-2 px-8 py-3 bg-[#00685F] hover:bg-[#004D46] text-white font-extrabold text-xs sm:text-sm rounded-2xl shadow-md shadow-[#00685F]/20 transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer active:scale-[0.98]"
          >
            {saving
