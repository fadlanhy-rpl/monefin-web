  ...
  return (
    <div className={`transition-all duration-700 ease-out transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} flex flex-col md:flex-row md:items-center justify-between gap-4`}>
      <div className="max-w-2xl">
        <h2 className="text-2xl lg:text-3xl font-extrabold text-slate-900 tracking-tight">{t("categories.title") || "Kategori Transaksi"}</h2>
        <p className="text-gray-400 text-xs sm:text-sm mt-2 leading-relaxed font-medium">
          {t("categories.subtitle") || "Kelola klasifikasi keuangan Anda dengan presisi. Atur anggaran dan pantau pengeluaran berdasarkan kategori yang dipersonalisasi."}
        </p>
      </div>
      <button 
      ...
      >
        <PlusCircle className="w-5 h-5 shrink-0" /> {t("categories.add_category") || "Add Category"}
      </button>
    </div>
  );
}
