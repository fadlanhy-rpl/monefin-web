...
export default function StatCards({ totalBalance = 0, totalIncome = 0, totalExpense = 0 }) {
  ...
  return (
    <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
        ...
        <div className="tilt-card shimmer-sweep glow-card relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-600 via-brand-700 to-brand-900 p-4.5 sm:p-6 text-white shadow-lg cursor-pointer">
          ...
          <div className="flex justify-between items-start relative z-10 gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-2xl sm:text-3xl xl:text-[1.65rem] 2xl:text-3xl font-extrabold tracking-tight tabular-nums min-h-[36px] sm:min-h-[40px] flex items-center font-mono sm:font-sans truncate mt-1.5 sm:mt-2">
                <CountUp target={totalBalance} isHidden={isBalanceHidden} />
              </p>
            </div>
            {/* Credit Card Chip Visual */}
            <div className="w-9 sm:w-10 h-7 sm:h-8 rounded-md bg-gradient-to-br from-amber-200 via-yellow-400 to-amber-300 p-1 opacity-90 shadow-sm flex flex-col justify-between shrink-0">
      ...
      <div className={`reveal card-hover rounded-2xl bg-white p-4.5 sm:p-6 shadow-card border border-slate-100/50 flex flex-col justify-between ${isVisible ? 'in-view' : ''}`} style={{ animationDelay: "80ms" }}>
        <div>
          <p className="flex items-center gap-2 text-xs sm:text-sm text-slate-600 font-semibold">
            <span className="w-1.5 h-1.5 rounded-full bg-brand-600 animate-pulse"></span> {t("dashboard.monthly_income") || "Monthly Income"}
          </p>
          <div className="flex flex-wrap items-baseline gap-2 mt-2 sm:mt-3">
            <p className="text-xl sm:text-2xl xl:text-xl 2xl:text-2xl font-extrabold text-slate-900 tracking-tight tabular-nums truncate max-w-full">
        ...
        <div className="mt-3.5 sm:mt-4 pt-2 flex items-end justify-between gap-3 sm:gap-4">
          <div className="flex-1 min-w-0">
            <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div className={`h-full bg-brand-600 rounded-full ${isVisible ? 'bar-grow' : ''}`} style={{ "--target-width": "78%" }}></div>
            </div>
            <p className="text-xs text-slate-600 mt-1.5 sm:mt-2 font-medium truncate">78% {t("dashboard.income_target") || "of income target achieved"}</p>
          </div>
          {/* SVG Sparkline */}
      ...
      <div className={`reveal card-hover rounded-2xl bg-white p-4.5 sm:p-6 shadow-card border border-slate-100/50 flex flex-col justify-between ${isVisible ? 'in-view' : ''}`} style={{ animationDelay: "160ms" }}>
        <div>
          <p className="flex items-center gap-2 text-xs sm:text-sm text-slate-600 font-semibold">
        ...
        <div className="mt-3.5 sm:mt-4 pt-2 flex items-end justify-between gap-3 sm:gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-600 mt-1.5 sm:mt-2 font-medium truncate">32% {t("dashboard.expense_limit") || "of monthly budget limit"}</p>
          </div>
