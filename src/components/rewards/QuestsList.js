...
import { Target, CheckCircle2, Gift, Sparkles, Clock, Check, Calendar, Zap } from "lucide-react";
    ...
    <div className="bg-white p-6 sm:p-8 rounded-[2.25rem] border border-slate-100 shadow-sm space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-teal-50 text-[#00685F] flex items-center justify-center shadow-xs">
              ...
              <div
                ...
                className={`p-4 sm:p-5 rounded-2xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
                  quest.is_claimed
                    ...
                    : canClaim
                    ? "bg-gradient-to-r from-emerald-50/90 via-teal-50/50 to-white border-emerald-300 ring-2 ring-emerald-400/20 shadow-sm"
              ...
              >
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 flex items-center gap-1">
                      {quest.type === "daily" ? (
                        <>
                    ...
                    <span className="text-xs font-black text-[#00685F] bg-emerald-100/80 px-2.5 py-0.5 rounded-full flex items-center gap-1">
                      <Zap className="w-3 h-3 fill-[#00685F]" />
                      +{quest.xp_reward} XP
                    </span>
                  </div>
                ...
                <div className="shrink-0 self-end sm:self-center">
                  {quest.is_claimed ? (
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-[#00A896] hover:from-emerald-700 hover:to-[#008f80] text-white font-extrabold text-xs sm:text-sm rounded-xl shadow-md shadow-emerald-700/20 active:scale-95 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
                    >
                      {claimingId === quest.id ? (
