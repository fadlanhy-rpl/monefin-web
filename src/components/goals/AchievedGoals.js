          ...
          >
            <div className="flex items-center gap-3 sm:gap-5 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-bold text-slate-900 text-sm sm:text-base truncate leading-tight">{ag.title}</p>
                <p className="text-[9px] sm:text-[10px] font-bold text-gray-400 italic truncate mt-0.5 select-none">Completed on {ag.completedDate}</p>
              </div>
            </div>
            <div className="text-right space-y-1 shrink-0 ml-1">
              <p className="text-sm sm:text-lg font-black text-slate-900 tracking-tight">Rp {ag.amount.toLocaleString("id-ID")}</p>
