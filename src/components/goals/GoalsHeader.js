  ...
  return (
    <div className={`transition-all duration-700 ease-out transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} space-y-4`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">{t("goals.title") || "Target Tabungan"}</h2>
          <p className="text-xs sm:text-sm font-bold text-[#00685F] mt-1 flex items-center gap-2 select-none">
            ...
            {activeGoalsCount} {t("goals.active_goals") || "Active Goals"}
          </p>
        </div>
        <button 
          type="button"
        ...
        >
          ...
          <span>{t("goals.create_new") || "Buat Target Baru"}</span>
        </button>
      </div>
    </div>
  );
